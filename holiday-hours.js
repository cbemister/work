...
// INIT CONTAINER VARS
var sidebarNoticeHTML = [],
	headerMessageText = '',
	footerMessageText = '';
// PARTIAL SIDE BAR MARK-UP
var noticeBody = [],
	noticeOpeningDiv = '<table class="hours hours--simple hours--holiday"><caption class="seo-hidden">Dealership hours of operation</caption><tbody>',
	noticeClosingDiv = '</tbody></table>';
...
jQuery(document).ready(function () {
	...
	if (holidayHoursEnabled && currentDepartmentHours) {
		...
		setHeaderFooterText(currentDepartmentHours); // IF CONDITION IS MET
		...
		jQuery('.hours-select').change(function () {
			...
			createSideBarNotice(currentDepartmentHours);
			setHeaderFooterText(currentDepartmentHours);
			jQuery('#hours--holiday').html(sidebarNoticeHTML); // REPLACE CURRENT HOLIDAY HOURS
		});
	}
...
function setHeaderFooterText(selectedHours) {
	if (selectedHours[today]) {
		...
		jQuery('.footer__content table.hours.hours--simple tbody').html(footerMessageText);
	}
}
...
function createSideBarNotice(selectedHours) {
	...
	sidebarNoticeHTML = '<h4 class="sidebar-title hours--holiday mt-md">Holiday Hours <i aria-hidden="true" class="fas fa-clock"></i></h4>' + noticeOpeningDiv + noticeBody.join('') + noticeClosingDiv;
}
